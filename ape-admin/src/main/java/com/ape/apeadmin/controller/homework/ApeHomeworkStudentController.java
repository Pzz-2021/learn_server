...
public class ApeHomeworkStudentController {
    ...
    @PostMapping("dianping")
    public Result dianping(@RequestBody JSONObject jsonObject) {
        String content = jsonObject.getString("content");
        String userId = jsonObject.getString("userId");
        String chapterId = jsonObject.getString("chapterId");
        UpdateWrapper<ApeHomeworkStudent> queryWrapper = new UpdateWrapper<>();
        queryWrapper.lambda().eq(ApeHomeworkStudent::getChapterId,chapterId)
                .eq(ApeHomeworkStudent::getUserId,userId).set(ApeHomeworkStudent::getRemark,content);
        boolean update = apeHomeworkStudentService.update(queryWrapper);
        if (update) {
            return Result.success();
        } else {
            return Result.fail();
