...
public class GenUpdateCode {
    ...
    public String genUpdate(ApeGenTable apeGenTable, List<ApeGenTableColumn> columnList) {
        ...
        builder.append("<template>\n" + "<div>\n" + "  <el-dialog title=\"编辑").append(apeGenTable.getTableComment()).append("\" width=\"40%\" :destroy-on-close=\"true\" :visible.sync=\"updateVisible\" :before-close=\"handleClose\">\n").append("    <el-form :model=\"form\" :rules=\"rules\" ref=\"ruleForm\">\n").append("      <el-row :gutter=\"10\">\n");
        ...
        builder.append("      </el-row>\n" + "    </el-form>\n" + "    <div slot=\"footer\" class=\"dialog-footer\">\n" + "      <el-button size=\"mini\" type=\"primary\" @click=\"submit\">确 定</el-button>\n" + "      <el-button size=\"mini\" @click=\"handleClose\">取 消</el-button>\n" + "    </div>\n" + "  </el-dialog>\n" + "</div>\n" + "</template>\n" + "\n" + "<script>\n" + "  import {edit").append(apeGenTable.getClassName()).append(",get").append(apeGenTable.getClassName()).append("ById} from '../../../api/api'\n").append("  export default {\n").append("    data() {\n").append("      return{\n").append("        formLabelWidth: '80px',\n").append("        form: {\n");
        for (ApeGenTableColumn apeGenTableColumn : columnList) {
            if (apeGenTableColumn.getIsInsert() == 1 && (!"id".equals(apeGenTableColumn.getJavaField()) && !"createBy".equals(apeGenTableColumn.getJavaField()) && !"createTime".equals(apeGenTableColumn.getJavaField()) && !"updateBy".equals(apeGenTableColumn.getJavaField()) && !"updateTime".equals(apeGenTableColumn.getJavaField()))) {
        ...
        builder.append("        this.$emit(\"updateFalse\")\n" +
                ...
                "      \n" +
                "    },\n");
        builder.append("    watch: {\n" + "      updateId(newVal) {\n" + "        if(newVal) {\n" + "          get").append(apeGenTable.getClassName()).append("ById({id:newVal}).then(res => {\n").append("            if(res.code == 1000) {\n").append("              this.form = res.data\n");
        for (ApeGenTableColumn apeGenTableColumn :columnList) {
            if ("Integer".equals(apeGenTableColumn.getJavaType())) {
                builder.append("              this.form.").append(apeGenTableColumn.getJavaField()).append(" = res.data.").append(apeGenTableColumn.getColumnName()).append(" + \"\"\n");
            }
        }
        builder.append("            } else {\n" +
                "              this.$notify.error({\n" +
